...
<template>
    <div>
        ...
        <div v-for="(exp, i) in experiences" :key="i" @click="selectExperienceHandler(exp)"
            class="bg-[#03101d] p-4 rounded-md text-sm space-y-1.5 my-2.5 hover:shadow-xl border-[#6EACDA] hover:border hover:cursor-pointer">
                ...
                <div class="w-[40rem] p-3 lg:p-6 flex flex-col space-y-5 flex-nowrap overflow-y-auto">
                    ...
                    <div class="px-4">
                        <ol v-for="(role, roleIdx) in selectedExperience.roles" :key="roleIdx" class="list-decimal">
                            <li>
                                {{ role.task }}
                                <ul v-for="(task, taskIdx) in role.subtask" :key="taskIdx" class="list-disc ml-5">
                                    <li>{{ task }}</li>
                                </ul>
                            </li>
                        </ol>
